...
const WorkspacePage: React.FC<WorkspacePageProps> = (props) => {
  ...
  if (user_workspaces_ctx) {
    const actual_workspace = user_workspaces_ctx.find(
      (w) => w.w_id === props.workspace_data.w_id
    );
    if (actual_workspace && user_task_ctx) {
      const newTaskHandler = async (t: ProgressStatusType) => {
        const data: TaskType = {
          ...
          w_id: actual_workspace.w_id,
          workspace_name: actual_workspace.name,
        };
      ...
      const task_list = user_task_ctx;

      const next_up = task_list.filter(
        (task) =>
          task.status === "NEXT-UP" &&
          task.w_id === encodeURIComponent(actual_workspace.w_id)
      );
      ...
      const menu_list: ButtonLargeProps[] = [
        {
          ...
          onClick: () => {
            router.push(`/workspace-setup/${actual_workspace.w_id}`);
          },
        },
        // {
        //   bg_color: "",
      ...
      return (
        <>
          ...
          <FormMenu
            ...
            submitHandler={(d) => {
              workspace_create_announcement_ctx(actual_workspace.w_id, {
                notification: {
                  message: d.value,
                  created_at: useDateNow(),
          ...
          <CardMenu
            ...
            title="Notification"
            notification_list={actual_workspace.notification_list}
          ...
          <BasicMenu
            ...
            log_list={actual_workspace.activity_list}
            closeHandler={() => {
          ...
          <main className={s.main}>
            <div className={s.task_board}>
              <OnlineBar users={actual_workspace.member_list} />
              <div
